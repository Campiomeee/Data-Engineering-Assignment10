...
with DAG(
    ...
    @task()
    def merge_csvs(persons_path: str, companies_path: str, output_dir: str = OUTPUT_DIR) -> str:
    ...
    @task()
    def load_csv_to_pg(conn_id: str, csv_path: str, table: str = "employees", append: bool = True) -> int:
        schema = "week8_demo"
    ...
    def clear_folder(folder_path: str = "/opt/airflow/data") -> None:
        """Delete all files/subdirs inside folder, but keep the folder (and keep /plots)."""
        ...
        for filename in os.listdir(folder_path):
            ...
            # Keep the plots directory
            if os.path.isdir(file_path) and os.path.basename(file_path) == "plots":
                print(f"Keeping directory: {file_path}")
                continue
    ...
    # @task()
    # def make_plot(top_domains: list[tuple[str, int]], outdir: str = "/opt/airflow/data/plots") -> str:
    #     """Save a simple bar chart and return its path."""
    #     import os
    #     import matplotlib.pyplot as plt
    ...
    #     plt.title("Top Company Email Domains (Employees)")
    #     plt.xticks(rotation=45, ha="right")
    #     plt.tight_layout()
    #     outpath = os.path.join(outdir, "top_domains.png")
    #     plt.savefig(outpath)
    #     plt.close()
    #     return outpath
    ...
    top_domains = analyze_from_pg(conn_id="postgres", table=TARGET_TABLE)
    plot_path   = make_plot(top_domains)

    load_to_database >> top_domains >> plot_path >> clean_folder